      ...
      /**********************************************************************/
        function inject_first_time_settings_to_database($conn, &$filters_table)
        {
          ...
          $sql_stmt = "INSERT INTO filters
            ...
            // executing sql.
                ...
                $buyer_filters_table["filter_id"] = $filters_table["filter_id"];
                $buyer_filters_table["buyer_id"] = $_SESSION['user_id'];

            // building custom variables because working with arrrays is buggy. php meeeh.

                $buyer_id =  $buyer_filters_table["buyer_id"] ;
                $filter_id = $buyer_filters_table["filter_id"]  ;
          $sql_stmt = "INSERT INTO buyer_filters_relation
                       (buyer_id, filter_id)
                       VALUES('$buyer_id', '$filter_id')";
            // executing sql.
                $result = $conn->query($sql_stmt);
        ...
        //**************************************************************************************//
         // fucntion checks if the user has changed the previous settings.
          ...
          /**********************************************************************************************/
            function print_data_in_buyer_filters_table($buyer_filters_table){
              echo "/------ data in buyer_filters_table ----------/";
