<?php
  include $db_connector;   //to connect to database... defined in the configuration file

  //now to get data from the database using property id(s) //

  //consider $_SESSION['admin_list_items'] has ids of properties in an array //
  for ($i=0; $i < count($_SESSION['admin_list_items']) ; $i++) {  //

      $property_id = $_SESSION['admin_list_items'][$i];   //acquitting the property_id in the array//
      ...
      include 'node_html.php';                 //contains the html string to be added dynamically to html form//
                                              //it adds a div of the class: 'result item' //

      array_push($new_nodes , $html_string);  //the $new_nodes array stores the html strings //
